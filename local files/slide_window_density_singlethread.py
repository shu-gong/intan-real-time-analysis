...
class SlideWindowBear():
    def __init__(self, left_hand, right_hand):
        ...
        self.margin_start_time = len(self.potential_threshold) * [0]
        self.margin_end_time = len(self.potential_threshold) * [0]
        self.margin_access_lock = len(self.potential_threshold) * [1]
...
class FindThresholdThread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            time.sleep(0.00001)
            print('current threshold******',slide_window_bear.num_margin_point)
            for i in range(len(slide_window_bear.potential_threshold)):
                if slide_window_bear.talk() > slide_window_bear.potential_threshold[i]:
                    if slide_window_bear.margin_aceess_lock == 1:
                        slide_window_bear.margin_start_time = int((time.time() - program_start_time) * 20* 1000)
                        slide_window_bear.margin_access_lock = 0
                else:
                    if slide_window_bear.margin_aceess_lock == 0:
                        slide_window_bear.margin_end_time = int((time.time() - program_start_time) * 20* 1000)
                        slide_window_bear.margin_access_lock = 1
            ...
            if time.time() - program_start_time > calibrate_time:
                ...
                for i in range(len(slide_window_bear.potential_threshold)):
                    for j in range(len(slide_window_bear.margin_points[i])):

                        # Calculate total time above threshold for each potential threshold
                        if j % 2 == 0 and j != 0:
                            slide_window_bear.time_above_threshold[i] += slide_window_bear.margin_points[i][j] - slide_window_bear.margin_points[i][j-1]
                
                # Inform the potential thresholds that meet the requirement
                for i in range(len(slide_window_bear.potential_threshold)):
                    if slide_window_bear.time_above_threshold[i] / calibrate_time > baseline:\
                        print("Potential Threshold: "+slide_window_bear.potential_threshold[i]+
                            ". Time above Threshold: "+ slide_window_bear.time_above_threshold[i])

                # Break the loop after calibration time
                break
...
class FoodCollectThread(threading.Thread):
    ...
    def run(self):
        ...
        cmd_socket.sendall(
            b'set '+channel_name.encode('utf-8')+b'.TCPDataOutputEnabled true;set '+channel_name.encode('utf-8')+b'.TCPDataOutputEnabledSpike true;set runmode run;')
